...
# Prints the database entries which will be getting deleted; set to False to avoid printing large lists and slowdown process  # noqa: E501
PRINT_DELETES = True
...
def print_configuration_function(**context):  # noqa: D103
...
print_configuration = PythonOperator(
    task_id="print_configuration", python_callable=print_configuration_function, provide_context=True, dag=dag
)


def cleanup_function(**context):  # noqa: D103
    ...
    try:
        ...
        if PRINT_DELETES:
            ...
            for entry in entries_to_delete:
                logging.info(
                    "\tEntry: " + str(entry) + ", Date: " + str(entry.__dict__[str(age_check_column).split(".")[1]])
                )
